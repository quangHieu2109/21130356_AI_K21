...
public class Puzzle {
	...
	public int computeH2(Node currentState) {
		...
		for (int i = 0; i < MAX_ROW; i++) {
			for (int j = 0; j < MAX_COL; j++) {
				int cur = currentState.getTile(i, j);
				int row = goalState.getLocation(cur)[0];
				int col = goalState.getLocation(cur)[1];
				int rowcur = i, colcur = j;
				if (cur != 0) {
					while (rowcur != row || colcur != col) {
						if (rowcur > row) {
							rowcur--;
							result++;
						} else if (rowcur < row) {
	...
	public Node moveWhiteTileH1(Node currentState, char operator) {
	...
	public List<Node> getSuccessorsH1(Node currentState) {
		ArrayList<Node> result = new ArrayList<Node>();

		for (char operator : operators) {
			Node tmp = moveWhiteTileH1(currentState, operator);
			if (tmp != null) {
				result.add(tmp);
			}
		}
